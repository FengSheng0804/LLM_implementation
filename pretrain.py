...
def init_model(lm_config, args):
    ...
    # 初始化模型
    model = MicroLM(lm_config).to(args.device)
    # 显示模型参数量
    show_logger(f'LLM总参数量：{sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.3f} 百万')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--wandb_project", type=str, default="MicroLM-Implementation")                              # wandb的项目名
